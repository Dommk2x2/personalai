...
const PieChartComponent: React.FC<PieChartComponentProps> = ({ transactions, expenseCategories, openingBalance }) => {
  const { currentThemeColors } = useTheme();
  ...
  const categoryColors = useMemo(() => {
    ...
    return colors;
  }, [expenseCategories, currentThemeColors.brandPrimary]);
  ...
  const CustomTooltip: React.FC<any> = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      const percentage = totalExpenses > 0 ? ((data.value / totalExpenses) * 100).toFixed(2) : 0;
      return (
        <div 
  ...
  if (expenseData.length === 0) {
    return (
      <div className="bg-bg-secondary-themed p-6 rounded-xl shadow-lg text-center text-text-muted-themed h-full flex flex-col justify-center items-center min-h-[400px]">
        <img src="https://picsum.photos/seed/piechartempty/300/200?grayscale" alt="Pie chart placeholder" className="mx-auto mb-4 rounded-lg w-32 h-24 sm:w-40 sm:h-32 md:w-48 md:h-32 object-cover opacity-70 dark:opacity-50" />
        <p>No expense data for category breakdown.</p>
        <p className="text-sm">Add expense transactions with categories to see a pie chart.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-bg-secondary-themed p-4 sm:p-6 rounded-xl shadow-lg h-[450px]"> 
      <h2 className="text-base font-semibold text-text-base-themed mb-3 text-center">Total Expenses</h2>
      <div className="flex h-[90%] w-full items-center">
        ...
        <div className="w-1/2 pr-4 overflow-y-auto h-full">
          <div className="flex justify-between items-baseline mb-2">
            ...
            <span className="text-[10px] text-text-muted-themed">Total: {expenseData.length}</span>
          </div>
              ...
              <li key={`item-${index}`} className="flex justify-between items-center text-xs gap-2 py-0.5">
                ...
                <div className="flex-shrink-0 flex items-baseline gap-1.5">
                    <span className="font-semibold text-text-base-themed text-[10px] text-right w-10">
                        {totalExpenses > 0 ? `${((entry.value / totalExpenses) * 100).toFixed(1)}%` : '0.0%'}
                    </span>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              >
                ...
                <Label
                  content={<CustomCenterLabel value={totalExpenses} openingBalance={openingBalance} />}
                  position="center"
                />
              </Pie>
