...
interface CategoryPriceMonitorProps {
...
const CategoryPriceMonitor: React.FC<CategoryPriceMonitorProps> = ({ transactions, expenseCategories, startDate, endDate }) => {
  ...
  const allCategories = useMemo(() => {
    ...
    transactions.forEach(tx => {
        if (tx.type === TransactionType.EXPENSE && tx.category) {
            categorySet.add(tx.category);
        }
    });
    // Filter against the master list of expense categories for consistency
    return expenseCategories.filter(cat => categorySet.has(cat));
  ...
  const totalsForSelected = useMemo(() => {
    ...
    const filtered = transactions
      .filter(tx => tx.type === TransactionType.EXPENSE && tx.category && selectedSet.has(tx.category));
  ...
  const chartData = useMemo(() => {
    // 1. Filter for expense transactions with a category
    const expenseTransactions = transactions.filter(
      tx => tx.type === TransactionType.EXPENSE && tx.category
    );
  ...
  const colors = currentThemeColors.categoryPillAccents;
  ...
  if (chartData.length === 0) {
     return (
      <div className="bg-bg-secondary-themed p-6 rounded-xl shadow-lg text-center text-text-muted-themed h-full flex flex-col justify-center items-center min-h-[400px]">
        ...
        <p className="text-sm">Add some expense transactions to monitor average category prices.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-bg-secondary-themed p-4 sm:p-6 rounded-xl shadow-lg h-[500px]">
      <div className="flex justify-between items-start">
        <div>
          <h2 className="text-lg font-semibold text-text-base-themed">Category Price Monitor</h2>
          <p className="text-xs text-text-muted-themed -mt-1 mb-4">Tracks the average transaction amount per category over time.</p>
        </div>
        <div className="flex items-center gap-4">
            {selectedCategories.length > 0 && (
                <div className="text-right">
                    <p className="text-xs font-medium text-text-muted-themed">Period Average</p>
                    <p className="font-bold text-lg" style={{ color: currentThemeColors.expense }} title="Average transaction of selected categories for the period">
      ...
      <ResponsiveContainer width="100%" height="75%">
        {chartType === 'line' ? (
            ...
            {selectedCategories.map((category, index) => (
                ...
                dot={{ r: 4 }}
                activeDot={{ r: 6 }}
                connectNulls // This will connect points across gaps where a category has no data for a month
                />
            ))}
            </LineChart>
        ) : (
...
export default CategoryPriceMonitor;
